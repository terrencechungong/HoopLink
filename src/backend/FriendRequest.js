...
export class FriendRequests {
    static async sendRequest(index, userFullName) {
        let receiver = document.getElementById(`email_${index}`).innerText;
        let recieverFullName = document.getElementById(`full_name_${index}`).innerText;
        let request = new FriendRequest(auth.currentUser.email, receiver, userFullName, recieverFullName);
    ...
    static async withdrawRequest(index) {
        let recipient = document.getElementById(`email_${index}`).innerText;
        const query_ = query(
            ref(database, "Users/"), orderByChild("receiver"), equalTo(recipient)
        );
        const response = await get(query_)
        response.forEach((snap) => {
            update(ref(database, "FriendRequest/" + snap.key), { is_active: false });
    ...
    static async getRelationshipData() {
        ...
        for (var chats of apiResponse.data) {
            for (var personData of chats.people) {
                ...
                if (personData.person.username == auth.currentUser.email) { 
                    userFullName = `${personData.person.first_name} ${personData.person.last_name}`
                    continue; 
                }
                noConnection_.push({
        ...
        return [noConnection_, sentRequests_, receivedRequests_, friends_, userFullName];
    }
}
